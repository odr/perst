...
type family BTreeHasNum s t where
  BTreeHasNum s (Node l n x r)
    = If (BTreeHas s l) '(1,IsLeaf l)
        (If (BTreeHas s r) '(2,IsLeaf r) '(0,False))

instance TDeleteB '(Just s==s1,False) s (Tagged (Leaf 1 s1) v)
      => TDelete (s::Symbol) (Tagged (Leaf 1 s1) v) where
  type DeletedType s (Tagged (Leaf 1 s1) v)
    = DeletedTypeB '(Just s==s1,False) s (Tagged (Leaf 1 s1) v)
  tdel = tdelB @('(Just s==s1,False)) @s

instance TDeleteB (BTreeHasNum s (Node l n t r)) s (Tagged (Node l n t r) v)
      => TDelete (s::Symbol) (Tagged (Node l n t r) v) where
  type DeletedType s (Tagged (Node l n t r) v)
    = DeletedTypeB (BTreeHasNum s (Node l n t r)) s (Tagged (Node l n t r) v)
  tdel = tdelB @(BTreeHasNum s (Node l n t r)) @s
...
instance TDeleteB '(1,True) s (Tagged (Node l n t r) (vl,vr)) where
  type DeletedTypeB '(1,True) s (Tagged (Node l n t r) (vl,vr)) = Tagged r vr
  tdelB = Tagged . snd . untag

instance TDeleteB '(2,True) s (Tagged (Node l n t r) (vl,vr)) where
  type DeletedTypeB '(2,True) s (Tagged (Node l n t r) (vl,vr)) = Tagged l vl
...
instance ( TDelete s (Tagged l vl)
      ...
      => TDeleteB '(1,False) s (Tagged (Node l (n::Nat) t r) (vl,vr)) where
  type DeletedTypeB '(1,False) s (Tagged (Node l n t r) (vl,vr))
    = Tagged (Node (TaggedTag (DeletedType s (Tagged l vl))) (n-1) t r)
...
instance ( TDelete s (Tagged r vr)
      ...
      => TDeleteB '(2,False) s (Tagged (Node l (n::Nat) t r) (vl,vr)) where
  type DeletedTypeB '(2,False) s (Tagged (Node l n t r) (vl,vr))
    = Tagged (Node l (n-1) t (TaggedTag (DeletedType s (Tagged r vr))))
