...
instance DDLConstr b (TableDef r p u f) => DDL b (TableDef r p u f) where
  createText pb pt
...
instance (DDLConstr b (ViewDef r s upd p u f), KnownSymbol s)
        => DDL b (ViewDef r s upd p u f) where
  createText pb pt
